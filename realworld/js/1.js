...
}})},load_trustmark:function(){var m=TrustedSite.config.host;var j=TrustedSite.config.trustmark.position_x;var a=TrustedSite.config.trustmark.position_y;
...
TrustedSite.load_js("https://www.trustedsite.com/rpc/ajax?do=tmjs-visit&host="+m+"&rand="+new Date().getTime())}}if(TrustedSite.is_mobile()&&TrustedSite.config.trustmark.disable_on_mobile){return
}if(document.getElementById("trustedsite-tm-image")){return}if(document.getElementById("trustedsite-tm-float-disable")){return
}if(document.getElementsByClassName("trustedsite-tm-float-disable").length>0){return}if(!TrustedSite.config.trustmark.enabled){return
...
}else{if([202,214,301,302,303,304,215].indexOf(h)>=0){if(TrustedSiteInline.config.brand!="ts"){return}a=TrustedSiteInline.calc_wh(a,120,50);
